...
@Controller // このクラスがWebページからのリクエストを受け取る「コントローラ」であることを示す
public class AuthController {
    // --- DI（依存性注入）でリポジトリとエンコーダを使えるようにする ---
    @Autowired
    ...
    public String showRegisterForm(Model model) {
        ...
        // トピックはユーザー登録段階では初期値なしだが、ユーザー情報編集画面でも同じ構造で設定できるようにするためnullをセット
        // いずれの項目も、ユーザー情報編集画面では現在の設定値（user.getDifficulty()など）を初期値にする
        userForm.setTopic(null);
        // モデルにフォームデータをセット
        model.addAttribute("userRegisterForm", userForm);
        // 希望条件選択用のリスト項目をビューに渡す
        ...
        return "register"; // templates/register.htmlを返す
    }
